...
/** 墙钟格式：<60s 显示 Xs，≥60s 显示 X分Y秒（对齐用户 footer 样例） */
...
function formatFooterField(id: FooterFieldId, session: CardSession): string | null {
  ...
  if (id === "elapsed") {
    ...
    return `用时 ${formatDurationSeconds(session.wallClockSeconds())}`;
  }
  if (id === "ttft") {
    if (typeof f.ttftAvgMs !== "number" || !Number.isFinite(f.ttftAvgMs)) return null;
    return `首 token 平均 ${(f.ttftAvgMs / 1000).toFixed(1)}s`;
  }
  if (id === "speed") {
    if (typeof f.outputSpeedTps !== "number" || !Number.isFinite(f.outputSpeedTps)) return null;
    return `${Math.round(f.outputSpeedTps)} tok/s`;
  }
  if (id === "cache_hit") {
