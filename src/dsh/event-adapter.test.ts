/**
 * event-adapter 核心映射测试：dsh session/event → 卡片事件流。
 */
...
describe("DshEventAdapter", () => {
  ...
  it("reasoning-delta → onThinkingDelta（showThinking 由渲染层控制，适配层总是转发）", () => {
    ...
    expect(env.streaming.onThinkingDelta).toHaveBeenCalledWith("思考中");
  });
  ...
  it("tool/call → onToolStart（arguments 为解析后对象）", () => {
    ...
    expect(env.streaming.onToolStart).toHaveBeenCalledWith("call-1", "bash", { cmd: "ls" });
  });
  ...
  it("tool/result → onToolEnd（提取文本 + 错误标志）", () => {
    ...
    expect(env.streaming.onToolEnd).toHaveBeenCalledWith("call-1", "ls 输出", false);
  });

  it("turn/end completed → settle + footer usage + onSettled", async () => {
    const env = makeEnv();
    await env.adapter["handleTurnEnd"](makeEvent("turn/end", {
  ...
  it("turn/end error → recordError + abort(llm_error)", async () => {
    ...
    expect(env.streaming.recordError).toHaveBeenCalledWith("rate limit");
    expect(env.streaming.abort).toHaveBeenCalled();
  });
  ...
  it("aborted 且卡片未终态 → abort 封卡后仍 settle + onSettled（P1-1 回归）", async () => {
    ...
    expect(env.streaming.abort).toHaveBeenCalledWith("任务已中断", "user_abort");
    expect(env.streaming.settle).toHaveBeenCalled();
